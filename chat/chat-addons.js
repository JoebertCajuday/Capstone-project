    ...
    {obj && (
        <>
            {(props.user?.user_id !== obj?.user_id) && obj?.id && ( 
                <Text style={{fontSize: 12, color: '#333'}}>{`${obj.username?.fullname} | ${obj.designation?.desig_name}`}</Text> 
            )}

            {obj.attachments && ( <ImageCont fileUrl={obj.attachments} key={obj.attachments}/> )}
